...
class LoginFormAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function authenticate(Request $request): Passport
    {
        ...
        $this->logger->debug('Login attempt', [
            'email' => $email,
            'has_password' => !empty($request->request->get('password', '')),
            'has_csrf' => !empty($request->request->get('_csrf_token', ''))
        ]);
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?RedirectResponse
    {
        ...
        $this->logger->info('User authenticated successfully', [
            'email' => $user instanceof UserInterface ? $user->getUserIdentifier() : 'unknown',
            'roles' => $user instanceof UserInterface ? implode(', ', $user->getRoles()) : []
        ]);
        
        if ($user instanceof UserInterface) {
            if (in_array('ROLE_ADMIN', $user->getRoles())) {
                $this->logger->debug('Admin detected: Redirecting to admin dashboard');
